    ...
    def define_schema(cls) -> io.Schema:
        return io.Schema(
            ...
            inputs=[
                ...
                io.Float.Input("spectrum_w", display_name="Blend", default=0.10, min=0.0, max=1.0, step=0.01, tooltip="画質と速度のバランスです。低いほど安全寄り、高いほど攻めた設定です。"),
                io.Int.Input("spectrum_warmup_steps", display_name="Warmup", default=6, min=0, max=50, step=1, tooltip="予測を始める前に実測するステップ数です。"),
                io.Int.Input("spectrum_window_size", display_name="Forecast Window", default=2, min=1, step=1, tooltip="実際に特徴を計算する間隔です。大きいほど高速ですが、ズレやすくなります。"),
                io.Boolean.Input("enable_calibration", display_name="Residual Calibration", default=False, tooltip="予測誤差を補正する処理を有効にします。オフのときは下の Calibration 設定は無視されます。"),
                io.Float.Input("calibration_strength", display_name="Calibration Strength", default=0.35, min=0.0, max=1.0, step=0.05, tooltip="Calibration 有効時に補正をどれだけ強く反映するかです。"),
                io.Combo.Input("calibration_mode", ["latest", "ema", "bucketed_ema"], display_name="Calibration Mode", default="ema", tooltip="補正値の蓄積方法です。"),
                ...
                io.Float.Input("spectrum_flex_window", display_name="Flex Window", default=0.0, min=0.0, max=1.0, step=0.01, tooltip="Warmup 後に予測ウィンドウを追加で広げる量です。", advanced=True),
                io.Int.Input("spectrum_stop_caching_step", display_name="Stop Forecast Step", default=-1, min=-1, max=200, step=1, tooltip="予測を止める最終ステップです。-1 で自動停止になります。", advanced=True),
